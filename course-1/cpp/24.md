...
А теперь запишем двойные указатели. Сначала все работает также, но, например, нельзя теперь указатель на Base** привести
к Derived**.
...
Пусть у нас есть массив int и массив const int.
...
` ` – читай произвольное количество символов.
...
Чтение может занимать линейное время (обычно от входа, так как, например, надо узнать длину строки).
