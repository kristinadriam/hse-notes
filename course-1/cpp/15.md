...
`#define` определяет макрос, который во что-то раскрывается. SOME_VAR —
переменная препроцессора.
...
Мораль: лучше не использовать предпроцессор, а если использовать, то аккуратно смотреть на все подвохи.
...
Нет рекурсии. Можно проэмулировать через жесткие костыли (Boost::PP).
