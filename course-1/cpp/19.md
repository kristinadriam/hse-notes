...
* std::cin >> input; – запись идет просто так, то есть producer может записывать когда угодно. Даже при чтении input из
  consumer. Фактически UB (чтобы поймать, надо чтобы cin записывал прямо одновременно с чтением из input, что очень
  маловероятно)
...
На курсе пользуемся библиотекой ```<boost/asio.hpp>```. Библиотека мощно, но будем использовать в простом, блокирующем
режиме (поток
...
io_context, tcp::endpoint(tcp::v4(), std::atoi(argv[1]))` – подключение, принимает контекст, где слушать (`tcp::()`
везде)) и кого слушать.
