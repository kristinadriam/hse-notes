...
функцией `std::make_index_sequence`
...
Решение зашито в стандарт отдельным костылем (в таком случае меняются правила вывода) и называется perfect forwarding.
...
2) `decltype(name)` – передана одна сущность, переменная/поле/аргумент (без скобок, static_cast, операций). Компилятор
   смотрит на то, как
