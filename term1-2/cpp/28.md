...
двух разных типов `T`,
то оно не скомпилируется. Но в данном случае при вызове 'MyTemplate<int>' нам придется определить `MyTemplate<void>`,
поэтому ODR все равно нарушится.
...
приходится дописывать `typename`.
...
Компилятор может по типу аругментов догадаться, какой тип у итератора, но не в обратную сторону (например, мы
...
С С++17 можно писать так, подключается CTAD (Class Template Argument Deduction). Например, можно не писать угловые
скобки у вектора.
